...
def main():
    ...
    # Launch with etrike_vehicle in foreground to see errors
    print("=== Launch etrike_vehicle (foreground, 60s) ===")
    ssh_run(client, "docker exec autoware_test bash -c 'source /opt/autoware/setup.bash && source /workspace/autoware/install/setup.bash && timeout 60 ros2 launch autoware_launch planning_simulator.launch.xml map_path:=/autoware_map/sample-map-planning vehicle_model:=etrike_vehicle sensor_model:=sample_sensor_kit rviz:=false 2>&1'", timeout=75)
