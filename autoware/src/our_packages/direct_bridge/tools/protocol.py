...
CONTRACTS = PROTOCOL_ROOT / "contracts"
VECTORS = PACKAGE_ROOT / "vectors"
GENERATED = PROTOCOL_ROOT / "generated"
...
def validate_model(model: dict, *, check_baseline: bool = True) -> dict:
    ...
    if ses_version and ses_version.get("layout", {}).get("semantic_support") != "raw_only":
...
def _validate_baseline(root: Path, instances: dict) -> None:
    baseline = read_json(PROTOCOL_ROOT / "contracts" / "baseline-manifest.json")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    generate.add_argument("--no-baseline", action="store_true", help=argparse.SUPPRESS)
