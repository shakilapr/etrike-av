...
static protocol::FrameView protocol_view(const struct can_frame & frame)
{
  if ((frame.can_id & (CAN_ERR_FLAG | CAN_RTR_FLAG)) != 0) {
    return protocol::FrameView(
      protocol::kExtendedCanIdMax + 1u, false, frame.len,
  ...
  return protocol::FrameView(
    frame.can_id & mask, extended, frame.len, frame.data,
    sizeof(frame.data));
}
...
void VehicleParams::validate_or_throw() const
{
  ...
  if (heartbeat_interval_ms <= 0) {
    throw std::domain_error("heartbeat_interval_ms must be positive");
  }
  if (rt_heartbeat_timeout_ms <= 0) {
    throw std::domain_error("rt_heartbeat_timeout_ms must be positive");
  }
  if (sys_status_timeout_ms <= 0) {
    throw std::domain_error("sys_status_timeout_ms must be positive");
  }
  if (state_report_timeout_ms <= 0) {
    throw std::domain_error("state_report_timeout_ms must be positive");
  }
  if (motion_report_timeout_ms <= 0) {
...
//  CanEncoder
// =====================================================================
CanEncoder::CanEncoder(const VehicleParams & params)
: params_(params) {}
  ...
  struct can_frame & frame)
{
  ...
  float steer = static_cast<float>(std::clamp(
      static_cast<double>(cmd.lateral.steering_tire_angle),
  ...
  struct can_frame & frame)
{
  ...
  if (turn) {
    message.left_turn = turn->command ==
      autoware_vehicle_msgs::msg::TurnIndicatorsCommand::ENABLE_LEFT;
    message.right_turn = turn->command ==
...
//  CanDecoder
// =====================================================================
bool CanDecoder::decode_velocity(
...
bool CanDecoder::decode_diagnostics(
  const struct can_frame & frame,
  ...
  const rclcpp::Time & now) const
{
  ...
  auto add = [&](const std::string & name, uint32_t value, uint8_t level,
      ...
      {diagnostic_msgs::msg::KeyValue kv; kv.key = "raw"; kv.value = std::to_string(value);
  ...
  add(
    "brake_engaged", brake_engaged, DiagnosticStatus::OK,
  ...
  add(
    "tec", tec,
    tec >=
    128 ? DiagnosticStatus::ERROR : (tec >= 96 ? DiagnosticStatus::WARN : DiagnosticStatus::OK),
    tec >= 128 ? "error-passive threshold" : (tec >= 96 ? "elevated" : "OK"));
  add(
    "rec", rec,
    rec >=
    128 ? DiagnosticStatus::ERROR : (rec >= 96 ? DiagnosticStatus::WARN : DiagnosticStatus::OK),
...
: rclcpp_lifecycle::LifecycleNode("vehicle_bridge", options)
{
  ...
  sub_turn_ = create_subscription<TurnIndicatorsCommand>(
    "/control/command/turn_indicators_cmd", command_qos,
    [this](const TurnIndicatorsCommand::SharedPtr m) {on_turn(m);});
  sub_hazard_ = create_subscription<HazardLightsCommand>(
    "/control/command/hazard_lights_cmd", command_qos,
    [this](const HazardLightsCommand::SharedPtr m) {on_hazard(m);});
  sub_engage_ = create_subscription<Engage>(
    "~/input/engage", command_qos,
  ...
  srv_control_mode_ = create_service<autoware_vehicle_msgs::srv::ControlModeCommand>(
    ...
    std::bind(
      &VehicleBridgeNode::on_control_mode, this, std::placeholders::_1,
      std::placeholders::_2));
  ...
  pub_mode_ = create_publisher<autoware_vehicle_msgs::msg::ControlModeReport>(
    "/vehicle/status/control_mode", rclcpp::QoS(1));
  pub_turn_status_ = create_publisher<autoware_vehicle_msgs::msg::TurnIndicatorsReport>(
    "/vehicle/status/turn_indicators_status", rclcpp::QoS(1));
  pub_hazard_status_ = create_publisher<autoware_vehicle_msgs::msg::HazardLightsReport>(
    "/vehicle/status/hazard_lights_status", rclcpp::QoS(1));
  pub_diag_ = create_publisher<diagnostic_msgs::msg::DiagnosticArray>(
    "~/output/diagnostics", rclcpp::QoS(
      1));
...
void VehicleBridgeNode::set_can_driver(std::unique_ptr<CanDriver> driver)
{
  can_ = std::move(driver);
}

// ---- Lifecycle ----
rclcpp_lifecycle::node_interfaces::LifecycleNodeInterface::CallbackReturn VehicleBridgeNode::
...
void VehicleBridgeNode::on_turn(
...
void VehicleBridgeNode::on_hazard(
...
void VehicleBridgeNode::on_emergency(
...
void VehicleBridgeNode::tick_control()
{
  ...
  if (cmd_age > params_.command_timeout_ms) {
    RCLCPP_WARN_THROTTLE(
      get_logger(),
      *get_clock(), 2000, "Command timeout: %.0fms — sending zero speed", cmd_age);
    accepting_control_.store(false, std::memory_order_relaxed);
    invalidate_control();
    send_safe_motion();
    return;
  ...
  bool braking = ctrl->longitudinal.is_defined_acceleration &&
...
void VehicleBridgeNode::tick_diagnostics()
{
  ...
  auto add = [&](const std::string & name, bool ok, const std::string & detail) {
      ...
      s.hardware_id = "etrike";
      s.level =
        ok ? diagnostic_msgs::msg::DiagnosticStatus::OK : diagnostic_msgs::msg::DiagnosticStatus::
        WARN;
      s.message = detail;
  ...
  add(
    "Confirmed mode", confirmed_auto_.load(std::memory_order_relaxed),
    confirmed_auto_.load(std::memory_order_relaxed) ? "AUTO" : "not AUTO");
  add(
    "Software emergency", !software_emergency_.load(std::memory_order_relaxed),
  ...
  add(
    "RT motion report", motion_fresh,
    motion_fresh ? "fresh" : "missing, frozen, invalid, or timeout");
  ...
  switch (protocol_view(frame).id()) {
    ...
    case messages::BrakeDiag::kId: {
        ...
        auto add_kv = [&](const std::string & name, double val, const std::string & unit) {
            ...
            {diagnostic_msgs::msg::KeyValue kv1; kv1.key = "value"; kv1.value = std::to_string(val);
              s.values.push_back(kv1); diagnostic_msgs::msg::KeyValue kv2; kv2.key = "unit";
              kv2.value = unit; s.values.push_back(kv2);}
            diag.status.push_back(s);
          };
