...
def render_outputs(model: dict, validated: dict) -> dict[str, str]:
    ...
    capabilities = {"schema_version": 1, "wire_hash": wire_hash, "languages": {}}
    ...
    return {
        ...
        "python/etrike_protocol.py": _render_python(metadata, wire_hash),
        "typescript/etrike-protocol.ts": _render_typescript(metadata, wire_hash),
        "cpp/etrike_protocol.hpp": _render_cpp(validated, wire_hash),
    }
...
def _cpp_enum_check(expression: str, field: dict, *, indent: str) -> list[str]:
    values = sorted((int(value) for value in field.get("enum", {})), key=int)
    if not values:
        return []
...
def _render_cpp(validated: dict, wire_hash: str) -> str:
    rows = []
    for key, message in sorted(validated["messages"].items()):
    ...
    lines = ["// Generated by protocol.tools.protocol. Do not edit.", "#pragma once", "#include <array>",
             ...
             "namespace etrike::protocol {", f'inline constexpr std::string_view kWireHash = "{wire_hash}";',
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "validate":
            wire_hash, network_hash = hashes(model, validated)
            print(f"valid: {len(validated['messages'])} messages, {len(validated['instances'])} instances, wire={wire_hash}, network={network_hash}")
        elif args.command == "generate":
            changed = write_or_check(render_outputs(model, validated), check=args.check)
