...
#
# Derived from tier4/nebula single_lidar_common_launch nebula_node_container.launch.py.
# Adds an optional calibration_file_path override and Hesai PTP parameters so a
...
def launch_setup(context, *args, **kwargs):
    ...
    # Calibration file: prefer an explicit override path, otherwise auto-resolve
    # from the vendored nebula_<make>_decoders/calibration/<model>.<ext>.
    calib_override = LaunchConfiguration("calibration_file_path").perform(context)
    if calib_override:
        sensor_calib_fp = calib_override
    elif sensor_extension is not None:
    ...
    container = ComposableNodeContainer(
        ...
        compositional_node_descriptions=nodes,
        output="both",
    )
...
def generate_launch_description():
    ...
    add_launch_arg("max_range", "300.0", "maximum view range")
    ...
    add_launch_arg("gnss_port", "10110", "device gnss port number")
    ...
    add_launch_arg(
        ...
        "",
        "optional explicit path to sensor calibration file; empty = auto-resolve from nebula decoders",
    )
    ...
    add_launch_arg("ptp_switch_type", "TSN", "PTP switch type")
    add_launch_arg("ptp_lock_threshold", "100", "PTP lock threshold")

    add_launch_arg(
        "distortion_correction_node_param_path",
        PathJoinSubstitution(
            [
                FindPackageShare("etrike_common_launch"),
                "config",
