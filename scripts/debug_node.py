...
def main():
    ...
    # Launch with output to see errors
    print("\n=== Launch with full output (15s timeout) ===")
    ssh_run(client, "docker exec autoware_test bash -c 'source /opt/autoware/setup.bash && source /workspace/autoware/install/setup.bash && timeout 15 ros2 launch etrike_vehicle_launch vehicle_interface.launch.xml can_interface:=vcan0 2>&1'", timeout=30)
