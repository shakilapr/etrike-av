...
def main():
    ...
    print("\n=== DIAGNOSTICS ===")
    ssh_run(client, "docker exec autoware_test bash -c 'source /opt/autoware/setup.bash && source /workspace/autoware/install/setup.bash && timeout 5 ros2 topic echo /vehicle_bridge/output/diagnostics --once 2>&1'")
    
    print("\n=== VELOCITY STATUS ===")
    ssh_run(client, "docker exec autoware_test bash -c 'source /opt/autoware/setup.bash && source /workspace/autoware/install/setup.bash && timeout 3 ros2 topic echo /vehicle/status/velocity_status --once 2>&1'")
