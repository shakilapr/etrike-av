...
def msg_to_dict(msg):
    return {
        "id": msg.id, "name": msg.name, "dlc": msg.dlc,
        "sender": msg.sender, "receivers": msg.receivers,
        "cycle_ms": msg.cycle_ms, "comment": msg.comment,
        "signals": [{
            "name": s.name, "byte": s.byte, "bit_offset": s.bit_offset,
            "size": s.size, "type": s.type.value if s.type else "unsigned",
...
<p class=subtitle>Generated from shared/can/can_{bus_name}.yaml &mdash; A3 landscape &mdash; {len(messages)} messages</p>
{body}
</body></html>"""
...
def main():
    ...
    generate_pdf('high', high, OUT_DIR / 'can_table_high.pdf')
    generate_pdf('low', low, OUT_DIR / 'can_table_low.pdf')
